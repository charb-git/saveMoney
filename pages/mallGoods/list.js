...
Page({
  ...
  onLoad: function (options) {
    mall_id = options.mall_id
    const mallName = options.mall_name
    pageIndex=1
  ...
  getPageData: function () {
    ...
    data.page_number = pageIndex
    utils.getDataByPost(data, "pdd.ddk.mall.goods.list.get", res => {
      const result = res.data
      if (result.goods_info_list_response && result.goods_info_list_response.goods_list.length > 0) {
        const tempList = result.goods_info_list_response.goods_list
